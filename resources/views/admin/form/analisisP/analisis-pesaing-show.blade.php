        ...
        <table class="w-full table-fixed">
            ...
            <tbody class="bg-white">
                @php
                    ...
                    $no++;
                @endphp
                @forelse ($pesaing as $Pesaing)
                <tr>
                    <td class="py-4 px-6 border-b border-gray-200">{{ $no }}</td>
                    <td class="py-4 px-6 border-b border-gray-200">{{ $Pesaing->product }}</td>
                    <td class="py-4 px-6 border-b border-gray-200">{{ $Pesaing->place }}</td>
                    <td class="flex gap-1 py-4 px-6 border-b border-gray-200">
                        <a href="{{ url('form/pesaing/edit/'.$Pesaing['id_user']) }}">
                            <span class="bg-blue-500 text-white py-1 px-2 rounded-full text-xs">Edit</span>
                        </a>  
                        <form action="{{ route('pesaing/delete', $Pesaing['id_user']) }}" method="POST">
                            @csrf
                            @method('DELETE')
